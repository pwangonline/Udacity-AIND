...
class PlanningGraph():
	...
	def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
		...
		return node_s1.symbol == node_s2.symbol and node_s1.is_pos != node_s2
	...
	def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
		...
		for parent in node_s1.parents | node_s2.parents:
	...
	def h_levelsum(self) -> int:
		...
		while counter < len(self.s_levels) and len(goals) > 0:
			for node in self.s_levels[counter]:
				if node.literal in goals:
					goals.remove(node.literal)
					level_sum += counter
			counter += 1
		return level_sum
