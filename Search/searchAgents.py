...
def foodHeuristic(state, problem):
	...
	l = [len(search.ucs(PositionSearchProblem(problem.gameState, start=position, goal=food, warn=False))) for food in
	     foodGrid.asList()]
	if len(l) == 0:
		return 0
	return max(l)
	...
	#     totalDist += len(search.bfs(PositionSearchProblem(problem.gameState, start=position, goal=food, warn=False)))
